...
const ProjectList = ({title, team, description, bgColor, link, logo, addClassName}:ProjectListProps) => {
  return (
    <li>
      <div className={`flex flex-col p-4 h-[18vh] md:grid md:grid-cols-2 justify-center items-start md:items-center ${bgColor} hover:opacity-80`}>
        <Link to={link} className="flex sm:ml-10 items-center">
          {logo}
          <div className={`text-3xl md:text-5xl font-medium ml-2 ${addClassName}`}>{title}</div>
        ...
        <p className="hidden md:block md:mr-28 font-notoKR whitespace-pre-line leading-relaxed">
          {description}
        </p>
      </div>
    </li>
